...
# 席作成
@router.post('/seats', response_model = SeatCreate)
...
# 席パターン作成
@router.post('/patterns', response_model = SeatPatternCreate)
def create_seat(pattern_data: SeatPatternCreate, db: Session = Depends(get_db)):
...
# 席パターンに使われている席作成
@router.post('/members', response_model = PatternMemberCreate)
def create_seat(member_data: PatternMemberCreate, db: Session = Depends(get_db)):
...
# 席情報取得
@router.get('/seats', response_model = list[SeatCreate])
def get_seats(db: Session = Depends(get_db)):
    return db.execute(select(Seat)).scalars().all()

# 席パターン取得
@router.get('/patterns', response_model = list[SeatPatternCreate])
def get_seats(db: Session = Depends(get_db)):
    return db.execute(select(SeatPattern)).scalars().all()

# 席パターンに使われている席情報取得
@router.get('/members', response_model = list[PatternMemberCreate])
def get_seats(db: Session = Depends(get_db)):
    return db.execute(select(PatternMember)).scalars().all()

# 席削除
@router.delete('/seat')
def delete_saet(seat_id: int, db: Session = Depends(get_db)):
